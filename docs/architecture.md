...
┌──────────────────────────────────────────────────────────────────┐
│                        UI 层 (Qt)                                │
│  AVDC_Main_new.py  ── QMainWindow, QTreeWidget, QProgressBar    │
│  Ui/AVDC_new.py    ── Qt Designer 编译后的 UI 代码              │
...
│  core/scrapers/  ── 7 个站点实现（javbus/javdb/jav321/...）      │
└──────────────────────────────────────────────────────────────────┘
```
...
- 同一套核心逻辑可同时服务于 GUI（`AVDC_Main_new.py`）和 CLI（`cli.py`）
...
| 入口 | 文件 | 说明 |
|------|------|------|
| GUI | `AVDC_Main_new.py` | PyQt5 窗口应用，`if __name__ == "__main__"` 启动 |
| CLI | `cli.py` | 无 Qt 依赖的命令行工具，支持 `--path`、`--single`、`--mode` 等参数 |

```bash
uv run python AVDC_Main_new.py              # 启动 GUI
