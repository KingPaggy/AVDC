...
def getActor(response):
    if re.search(r'<a href="/star/\S+">(\S+)</a> &nbsp;', response):
        return (
            str(re.findall(r'<a href="/star/\S+">(\S+)</a> &nbsp;', response))
            .strip(" [',']")
            .replace("'", "")
        )
    elif re.search(r'<a href="/heyzo_star/\S+">(\S+)</a> &nbsp;', response):
        return (
            str(re.findall(r'<a href="/heyzo_star/\S+">(\S+)</a> &nbsp;', response))
            .strip(" [',']")
            .replace("'", "")
        )
    else:
        return (
            str(re.findall(r"<b>出演者</b>: ([^<]+) &nbsp; <br>", response))
            .strip(" [',']")
            .replace("'", "")
        )
...
def getSeries(response):
    return str(
        re.findall(r'<b>系列</b>: <a href="/series/\S+">(\S+)</a>', response)
    ).strip(" ['']")
...
def getRelease(response):
    return (
        str(re.findall(r"<b>配信開始日</b>: (\d+-\d+-\d+)<br>", response))
        .strip(" ['']")
        .replace("0000-00-00", "")
    )
...
def getCover(detail_page):
    ...
    if cover_url == "":
        cover_url = str(
            detail_page.xpath("//*[@id='vjs_sample_player']/@poster")
        ).strip(" ['']")
    return cover_url
...
def getOutline(detail_page):
    return str(
        detail_page.xpath("/html/body/div[2]/div[1]/div[1]/div[2]/div[3]/div/text()")
    ).strip(" ['']")
