...
def getActorPhoto(htmlcode):
    ...
    for i in a:
        actor_link = i.a["href"]
        t = i.get_text()
        html = etree.fromstring(get_html(actor_link), etree.HTMLParser())
        p = "https://javbus.com" + str(
...
def getNum(htmlcode):  # 获取番号
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = str(
        html.xpath('//span[contains(text(),"識別碼")]/following-sibling::span/text()')
    ).strip(" ['']")
    return result
    ...
    try:
        ...
        if outline == "":
            ...
            if "に一致する商品は見つかりませんでした" not in dmm_htmlcode:
                ...
                url_detail = (
                    str(dmm_page.xpath('//*[@id="list"]/li[1]/div/p[2]/a/@href'))
                    .split(",", 1)[0]
                    .strip(" ['']")
                )
                if url_detail != "":
                    ...
                    outline = (
                        str(html.xpath('//*[@class="mg-t0 mg-b20"]/text()'))
                        .strip(" ['']")
                        .replace("\\n", "")
                        .replace("\n", "")
                    )
        ...
        for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
            ...
            if len(number_get) > 0 and number_get[0].upper() == number.upper():
                cover_small = html.xpath(
                    "//div[@id='waterfall']/div["
                    + str(count)
                    + "]/a/div[@class='photo-frame']/img/@src"
                )[0]
                return cover_small
    except Exception as error_info:
...
def find_number(number):
    ...
    # =======================================================================无码搜索
    htmlcode = get_html(
        "https://www.javbus.com/uncensored/search/" + number + "&type=1"
    )
    ...
    for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
        ...
        if (
            ...
            return result_url
        elif number_get == number.replace("-", "_") or number_get == number.replace(
            "_", "-"
        ):
            result_url = html.xpath(
                "//div[@id='waterfall']/div[@id='waterfall']/div["
    ...
    try:
        if appoint_url:
            ...
            for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
                ...
                if (
                    number_get.upper() == number.upper()
                    or number_get.replace("-", "").upper() == number.upper()
                ):
                    result_url = html.xpath(
                        "//div[@id='waterfall']/div["
                        + str(count)
