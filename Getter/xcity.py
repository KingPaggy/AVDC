...
def getTitle(a):
    ...
    result = (
        str(html.xpath("//span[@id='program_detail_title']/text()"))
        .strip(" ['']")
        .replace("'", "")
    )
    return result
...
def getActor(
    ...
    html = etree.fromstring(a, etree.HTMLParser())
    result = (
        str(html.xpath("//li[@class='credit-links']/a/text()"))
        .strip(" ['']")
        .replace("'", "")
    )
    return result
...
def getTag(a):
    html = etree.fromstring(a, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = (
        str(html.xpath("//a[@class='genre']/text()"))
        .strip(" ['']")
        .replace("'", "")
        .replace("\\t", "")
        .replace("\\n", "")
...
def getCover(htmlcode):
    ...
    result = "https:" + str(
        html.xpath("//div[@class='photo']/p[@class='tn']/a/@href")
    ).strip(" ['']").replace("'", "")
    return result
...
def getDirector(a):
    html = etree.fromstring(a, etree.HTMLParser())
    result1 = (
        str(html.xpath("//span[@id='program_detail_director']/text()"))
        .strip(" ['']")
        .replace("\\t", "")
        .replace("\\n", "")
    )
...
def getOutline(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result = (
        str(html.xpath("//p[@class='lead']/text()"))
        .strip(" ['']")
        .replace("\\n", "")
        .replace("\n", "")
    )
...
def find_number(number, appoint_url):
    ...
    htmlcode = get_html(
        "https://xcity.jp/result_published/?q=" + number.replace("-", "")
    )
