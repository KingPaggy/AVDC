...
def getStudio(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"片商:")]/../span/a/text()')
    ).strip(" ['']")
    result2 = str(
...
def getPublisher(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"發行:")]/../span/a/text()')
    ).strip(" ['']")
    result2 = str(
...
def getSeries(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"系列:")]/../span/a/text()')
    ).strip(" ['']")
    result2 = str(
...
def getNumber(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())
    result1 = (
        str(html.xpath('//strong[contains(text(),"番號:")]/../span/a/text()'))
        .strip(" ['']")
        .replace("_", "-")
    )
    result2 = (
        str(html.xpath('//strong[contains(text(),"ID:")]/../span/a/text()'))
        .strip(" ['']")
        .replace("_", "-")
    )
...
def getRelease(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"日期:")]/../span/text()')
    ).strip(" ['']")
    result2 = str(
...
def getTag(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"類別:")]/../span/a/text()')
    ).strip(" ['']")
    result2 = str(
...
def getCover(htmlcode):
    ...
    if not result:
        soup = BeautifulSoup(
            htmlcode, "lxml", parse_only=SoupStrainer("img", {"class": "video-cover"})
        )
...
def getDirector(htmlcode):
    html = etree.fromstring(htmlcode, etree.HTMLParser())  # //table/tr[1]/td[1]/text()
    result1 = str(
        html.xpath('//strong[contains(text(),"導演:")]/../span/a/text()')
    ).strip(" ['']")
    result2 = str(
...
def getOutlineScore(number):  # 获取简介
    ...
    try:
        ...
        if re.search(
            r'<b>平均評価</b>: <img data-original="/img/(\d+).gif" />', response
        ):
            score = re.findall(
                r'<b>平均評価</b>: <img data-original="/img/(\d+).gif" />', response
            )[0]
            ...
            score = (
                str(re.findall(r"<b>平均評価</b>: ([^<]+)<br>", response))
                .strip(" [',']")
                .replace("'", "")
            )
...
def main(number, appoint_url, isuncensored=False):
    try:
        if appoint_url == "":
            # ========================================================================搜索番号
            htmlcode = get_html_javdb(
                "https://javdb.com/search?q=" + number + "&f=all"
            ).replace("\xa0", " ")
            if str(htmlcode) == "ProxyError":
                raise TimeoutError
            html = etree.fromstring(
                htmlcode, etree.HTMLParser()
            ...
            result_url = (
                "https://javdb.com"
                + html.xpath('//*[@id="videos"]/div/div/a/@href')[count - 1]
            )
        else:
            result_url = appoint_url
        # ========================================================================请求、判断结果
        html_info = get_html_javdb(result_url + "?locale=zh").replace("\xa0", " ")
        ...
        outline = ""
        if isuncensored and (
            re.match(r"^\d{4,}", number) or re.match(r"n\d{4}", number)
        ):  # 无码，收集封面、评分
            imagecut = 0
        ...
        if appoint_url:
            ...
            for count in range(1, counts + 1):  # 遍历搜索结果，找到需要的番号
                series_get = html.xpath(
                    "//div[@id='videos']/div[@class='grid columns']/div[@class='grid-item column']["
                    + str(count)
                    + "]/a[@class='box']/div[@class='uid2']/text()"
                )[0]
                if not series_get:
                    series_get = html.xpath(
                ...
                date_get = html.xpath(
                    "//div[@id='videos']/div[@class='grid columns']/div[@class='grid-item column']["
                    + str(count)
                    + "]/a[@class='box']/div[@class='meta']/text()"
                )[0]
                if not date_get:
                    date_get = html.xpath(
                        "//div[@id='videos']/div[@class='grid columns']/div[@class='grid-item column horz-cover']["
                ...
                if (
                    series_get.upper() == number_series.upper()
                    or series_get.replace("-", "").upper() == number_series.upper()
