...
class MyMAinWindow(QMainWindow, Ui_AVDV):
    ...
    def Load_Config(self):
        ...
        self.Ui.lineEdit_uncensored_prefix.setText(
            config["uncensored"]["uncensored_prefix"]
        )
        # ========================================================================file_download
        ...
        self.Ui.lineEdit_extrafanart_dir.setText(
            config["extrafanart"]["extrafanart_folder"]
        )
    ...
    def deletethumb(self, path, naming_rule):
        try:
            ...
            if (not self.Ui.checkBox_download_thumb.isChecked()) and os.path.exists(
                thumb_path
            ):
    ...
    def extrafanartDownload(self, json_data, path, Config, filepath, failed_folder):
        ...
        if (
            self.Ui.radioButton_extrafanart_download_on.isChecked()
            and str(json_data["extrafanart"]) != ""
        ):
    ...
    def add_mark(self, poster_path, thumb_path, cn_sub, leak, uncensored, config):
        ...
        if self.Ui.checkBox_uncensored.isChecked() and uncensored:
            mark_type += ",无码"
        if (
            self.Ui.radioButton_thumb_mark_on.isChecked()
            and mark_type != ""
            and self.Ui.checkBox_download_thumb.isChecked()
        ...
        if (
            self.Ui.radioButton_poster_mark_on.isChecked()
            and mark_type != ""
            and self.Ui.checkBox_download_poster.isChecked()
    ...
    def add_mark_thread(self, pic_path, cn_sub, leak, uncensored):
        size = 14 - int(
            self.Ui.horizontalSlider_mark_size.value()
        )  # 获取自定义大小的值
    ...
    def Core_Main(self, filepath, number, mode, count, appoint_url=""):
        ...
        success_folder = (
            movie_path + "/" + self.Ui.lineEdit_success.text()
        )  # 成功输出目录
        # =======================================================================获取json_data
